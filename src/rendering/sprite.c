...
static void	calc_sprite_transform(t_game *game, t_sprite_render *sr,
	int i, double scale)
{
	double	sprite_x;
	double	sprite_y;
	...
	sr->width = abs((int)(HEIGHT / sr->transform_y * scale));
}
	...
	double vground, double transform_y)
{
	...
	if (sr->draw_start_y < 0)
		sr->draw_start_y = 0;
	sr->draw_end_y = sr->height / 2 + HEIGHT / 2 + pitch + v_screen;
...
static t_img	*get_collectible_tex(t_game *game, int i)
{
	int	type;
...
static void	render_single_sprite(t_game *game, int i, double *z_buffer)
{
	...
	if (sr.transform_y <= 0.1)
		return ;
	calc_sprite_bounds(&sr, game->player.pitch, vground, sr.transform_y);
